...
所得分数按在 CryptoHack 所设定的 points 和给出，会根据大家提交的作业的 points 采用合适的方式换算出最终得分。
...
的点 $(x,y)$ 组成的集合，注意 $x^3+ax^2+bx+c=0$ 需有 3 个不同解。(当然，椭圆曲线还有其他形式，这里是Weierstrass形式，相互间通过对坐标的线性变化得到同态映射)
