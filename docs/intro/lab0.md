...
Lab 过程中遇到的无法解决的问题或者其他任何与课程相关的问题都欢迎加入课程交流群进行讨论，QQ 群号 876253830。
...
???+ note "环境推荐"
    ...
    关于 Linux 发行版，我们推荐 Debian 13 "Trixie" / Ubuntu 26.04 LTS "Resolute Raccoon" / Kali Linux 2026.1 等。
...
!!! tip "Hint"
    - Linux 环境的准备可以参考[2022年的课程作业](https://github.com/team-s2/ctf_summer_courses/blob/2022/homework/trivial/01_linux_hw.pdf)
...
这里选取一个比较经典的题：大家可以前往 [CTF^2](https://ctf2.dasctf.com/) 平台，在能力训练-公开练习场中搜索找到 **Hack World** 这道题，完成它并在实验报告中记录过程。

顺带一提，BUUCTF 是一个很不错的公开练习平台！不过最近 BUUCTF 已被归档，原本的题目均迁移至本题所述的新平台 CTF^2 中，然而新平台由于~~vibe coding~~处于测试阶段，目前稳定性不是很好，对 firefox 等浏览器的前端支持也存在诸多 bug，请同学们尽量不要过于贴近 DDL 做本题，以防不可控情况发生。
...
```text
;cR!f<`MjL9eg<R<bl5:;D9go<,6#?<CUXo<)lXK;cPZ";D9h.=);&:<bu;I9l*Er<A8VO<GYQ"<blqZ<%p0t<bbf5<GY,q;cHL`9kl@B:f0A=
```
...
[CryptoHack](https://cryptohack.org/) 是一个非常适合自学密码学知识的题目平台，你需要注册并完成 [CryptoHack - General challenges](https://cryptohack.org/challenges/general/) 中 **Mathematics** 板块的五道基础数学知识题（Greatest Common Divisor 至 Modular Inverting），在实验报告中给出解题过程。
