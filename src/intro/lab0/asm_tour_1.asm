...
; Welcome to ACTF! And, presumably, welcome to reverse engineering.
; This challenge is designed for N3WbS, so don't be nervous if you are a freshman and don't know what reverse engineering or assembly is. 

; Rule # ONE :
;  Since this problem is designed to introduce newbies to reverse engineering, this source document is designed such that you can hopefully read it straight down and come to a reasonable understanding of this nonsense...
...
;    For example, if you see a call to `print("input: ")` then a call to `password = inputPassword()`, I would sincerely hope that you could deduce that you're being asked to input the weight of an average-sized giraffe.  
...
; It's worth mentioning that assembly is a dense topic, there's a lot here to understand... You should probably go take a course on this and read some book and --- WAIT!! NO!! THIS IS ACTF!!! WE LEARN THIS THE FUN WAY; MISGUIDEDLY DIVING IN HEAD FIRST AND BREAKING SHIT ALONG THE WAY.
; So here we go.

; To start, I am not going to give you a hello world program as is standard - that would be far too complicated a program to parse.  Let's instead write a basic operating system.  Much easier, right? *eye twitch*
...
;
;  4. Probably a text editor to edit text like this challenge.  I like VisualStudioCode.. It has some 'asm' extentions.
;
...
;
;  6. To practically advance in reverse engineering, you'll need a disassembler.
...
_start:  ; This is a label...  It's not an instruction.  It is simply a marker for us humans.  We put it here so we can reference it later, such that ff we later specify `_start` in our code, NASM will know we're referencing the element immediately following the label.
  ...
  mov gs, dx ; to use them to help me clear
...
stack_example:
  ...
  ; Before we leave this w0dERful souRCE cod3, I want to thank you for coming to ACTF and reading this..
  ...
  ; 
  ; Submission tips: Append your answers (chars in lower case) after We1com3_7o_R3_, seperated by '_', inside ACTF{} format. 
  ; For example, suppose your answers are 00, aa, bbbb, ffff, 0000, 0000, respectively;
  ; Then the flag you are going to submit is:
